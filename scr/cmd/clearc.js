...
module.exports.start = async function({ api, event }) {
  ...
  if (result === "burat") {
    return api.sendMessage("Cache folder is empty to delete", event.threadID, event.messageID);
  } else {
    return api.sendMessage("Deleting cache successfully", event.threadID, event.messageID);
  }
}